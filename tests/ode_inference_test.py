...
def test_repressilator_simulation_class():
    """
    ...
    test_simulation= np.loadtxt(os.path.join(testdata_path, "simulation_003.txt"))
    test_time_seconds=test_simulation[:,0]
    test_nuclear_protein=test_simulation[:,2]
    test_cytosol_protein=test_simulation[:,1]
    with open(os.path.join(testdata_path, "parameters", "params_003.json"),"r") as f:
        params=json.load(f)
    ...
    assert RMSE(values[:,0], test_nuclear_protein)<36
    assert RMSE(values[:,1], test_cytosol_protein) <36
def test_infer_parameters():
    ...
    assert RMSE(sim[:,0], true_array[:,1])<36
    assert RMSE(sim[:,1], true_array[:,2])<36
